...
function user_type_change(){
	...
	if($("#user_type").val()=="学员"||$("#user_type").val()=="弟子"){
		$("#realname").text(" * ");
		if($("#user_type").val()=="弟子"){
			$("#birthday").text(" * ");
			$(".picture").show();
	...
	if($("#user_type").val()=="普通"){
		...
		$(".picture").hide();
		$(".introduce").hide();
		$("#null_realname").css("display","none");
		$("#null_date").css("display","none");
	}
}
...
//检查密码的长度
function check_pw1(){
	$("#null_password").css("display","none");
	if($("#password1").val().length!=0 && $("#password1").val().length<6){
		$("#wrong_pw1").css("display","block");
	}else{
		$("#wrong_pw1").css("display","none");
		$("#password2").focus();
	}
}
//检查两次输入的密码是否一致
function check_pw2(){
	if($("#password2").val()!=$("#password1").val()){
		$("#wrong_pw2").css("display","block");
		return false;
	}else{
		$("#wrong_pw2").css("display","none");
	}
}
...
function check_null(){
	...
	if($("#user_type").val()=="弟子" && $("input[name='birthday']").val()==""){
		$("#null_date").css("display","block");
	}
	if($("input[name='username']").val()!="" && $("#password1").val()!=""){
		if(($("#user_type").val()=="学员" ||$("#user_type").val()=="弟子") && $("input[name='realname']").val()!=""){
			if($("#user_type").val()=="弟子" && $("input[name='birthday']").val()!="")
				return true;
			else
