	...
	<script>
		...
		function check(){
			if($("#con1").val()==""){
				...
				var obj={
					url:'${pageContext.request.contextPath}/message_update.action',
					type:'post',
		...
		$(function(){
			$("textarea").val('${param.message_content}');
		});
	</script>
  </head>
    	...
    	<form action="" method="post">
    		<table class="tb">
    			<tr>
    				<td>信息标题：</td>
    				<td>
    					<input type="text" name="con1" id="con1" value="${param.message_title }" onblur="disappear(this)">
    					<span class="tip1">请输入标题</span>
    				</td>
    			</tr>
    			<tr>
    				<td>信息内容：</td>
    		...
    		<input type="hidden" value="${param.messageId }" name="messageId">
    		<input type="hidden" name="publishTime" id="publishTime">
    		<input type="hidden" value="${param.author }" name="author">
    		<div class="btn">
