	...
	$.fn.jqGrid = function(pin) {
		...
		return this.each(function() {
			...
			var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
				...
				addXmlData = function(xml, rcnt, more, adjust) {
					...
					var self = $(ts),
						...
						cn1 = getstyle(stylemodule, 'rowBox', true, 'jqgrow ui-row-' + ts.p.direction),
						classes;
			...
			for(i = 0; i < this.p.colNames.length; i++) {
				...
				thead += "<div class='ui-th-div' id='jqgh_" + ts.p.id + "_" + tmpcm.name + "' " + tdc + ">" + (tmpcm.name == 'rn' ? '行号' : ts.p.colNames[i]);
				if(!tmpcm.width) {
					tmpcm.width = 150;
				} else {
			...
			if(ts.p.pager) {
				...
				$(ts.p.pager).css({
					width: grid.width + "px"
				}).addClass(getstyle(stylemodule, 'pagerBox', true, 'ui-jqgrid-pager')).appendTo(eg);
				if(hg) {
					$(ts.p.pager).hide();
				}
				setPager(ts.p.pager, '');
			...
			if(ts.p.direction === 'rtl') {
				$(ts).bind('jqGridAfterGridComplete.setRTLPadding', function() {
					...
					$("div:first", grid.hDiv).css({
						paddingLeft: vScrollWidth + "px"
					});
					//} else {
	...
	$.jgrid.extend({
				...
				if(!$t.p.multiselect) {
					if(pt.className !== "ui-subgrid") {
						if($t.p.selrow !== pt.id) {
							...
							if(csr) {
								$(csr).removeClass(highlight).attr({
									"aria-selected": "false",
									"tabindex": "-1"
								});
							}
							$(pt).addClass(highlight).attr({
								"aria-selected": "true",
								"tabindex": "0"
							}); //.focus();
							if(fid) {
		...
		setGridWidth: function(nwidth, shrink) {
			return this.each(function() {
				...
				var $t = this,
					...
					aw, gw = 0,
					cr, bstw = $t.p.styleUI === 'Bootstrap' ? 2 : 0;
		...
		setFrozenColumns: function() {
			return this.each(function() {
				...
				if($t.p.subGrid === true || $t.p.treeGrid === true || $t.p.sortable || $t.p.scroll) {
					return;
				}
				if($t.p.rownumbers) {
					i++;
				...
				if(maxfrozen >= 0 && frozen) {
					...
					if($t.p.footerrow) {
						...
						console.log($(".ui-jqgrid-ftable", "#gview_" + $.jgrid.jqID($t.p.id)));
					...
					$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function() {
						...
						$($t.grid.fbDiv).height($($t.grid.bDiv).height() - 17); //减去滚动条的高度 
						// find max height
						var mh = [];
						...
						case 'custom':
							try {
								...
								if(e === "e1") {
									...
									$.jgrid.info_dialog(errors.errcap, "function 'custom_value' " + edit.msg.novalue, edit.bClose, {
										styleUI: $t.p.styleUI
									});
								} else {
	...
	$.extend($.jgrid, {
		...
		viewModal: function(selector, o) {
			...
			if($.fn.jqm && o.jqm === true) {
				...
				if(o.focusField >= 0) {
					try {
						$(':input:visible', selector)[parseInt(o.focusField, 10)].focus();
					} catch(_) {}
				}
			}
		},
			...
			switch(eltype) {
				...
				case "custom":
					...
					try {
						...
						if(e === "e1") {
							$.jgrid.info_dialog(errors.errcap, "function 'custom_element' " + edit.msg.nodefined, edit.bClose, {
								styleUI: $t.p.styleUI
							});
						} else if(e === "e2") {
							$.jgrid.info_dialog(errors.errcap, "function 'custom_element' " + edit.msg.novalue, edit.bClose, {
								styleUI: $t.p.styleUI
							});
						} else {
							$.jgrid.info_dialog(errors.errcap, typeof e === "string" ? e : e.message, edit.bClose, {
								styleUI: $t.p.styleUI
							});
						}
					}
				...
				default:
					...
					if(eltype !== "button") {
						if(autowidth) {
							if(!options.size) { //修改编辑状态下input的宽度
								$(elem).css({
									width: "100%",
									height: "100%",
									padding: "0px",
			...
			*/
			...
			this.createTableForGroup = function(group, parentgroup) {
				...
				if(this.p.ruleButtons === true) {
					...
					var inputAddRule = $("<input type='button' value='+' title='Add rule' class='add-rule ui-add " + common.button + "'/>"),
						cm;
			...
			this.createTableRowForRule = function(rule, group) {
				...
				var ruleFieldSelect = $("<select class='" + classes.srSelect + "'></select>"),
					ina, aoprs = [];
...
//						stbl = $("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-oper' headers=''></td><td class='ui-search-input' headers=''></td><td class='ui-search-clear' headers=''></td></tr></table>");
						stbl = $("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-input' headers=''></td><td class='ui-search-oper' headers=''></td><td class='ui-search-clear' headers=''></td></tr></table>");
					if(this.hidden === true) {
						$(th).css("display", "none");
					}
					...
					if(this.search) {
						...
						if(p.searchOperators) {
							...
							select = this.searchoptions.searchOperMenu ? "<a title='" + st + "'  soper='" + so + "' class='soptclass' colname='" + this.name + "'></a>" : "";

						}
//						$("td:eq(0)", stbl).attr("colindex", ci).append(select);//过滤选项
						$("td:eq(1)", stbl).attr("colindex", ci).append(select);//过滤选项
						if(soptions.clearSearch === undefined) {
							soptions.clearSearch = true;
						}
						if(soptions.clearSearch) { //搜索列清除按钮
							//						csv = p.resetTitle || 'Clear Search Value';
							//	$("td:eq(2)",stbl).append("<a title='"+csv+"' style='padding-right: 0.3em;padding-left: 0.3em;' class='clearsearchclass'>"+p.resetIcon+"</a>");
							$("td:eq(2)", stbl).append("<a  class='clearsearchclass'>" + p.resetIcon + "</a>");
						} else {
							$("td:eq(2)", stbl).hide();
						}
...
//						$("td:eq(1)", stbl).append(elem);//输入框
						$("td:eq(0)", stbl).append(elem);//输入框
						$(thd).append(stbl);
...
//						$("td:eq(0)", stbl).hide();//过滤项
						$("td:eq(1)", stbl).hide();//过滤项
					}
				});
		...
		refreshFilterToolbar: function(p) {
			...
			return this.each(function() {
				var $t = this,
					cm = $t.p.colModel,
					i, l = $t.p.colModel.length,
					searchitem, filters, rules, rule, ssfield = [],
					ia;
		...
		filterInput: function(val, p) {
			...
			return this.each(function() {
				...
				var nm, sop, ruleGroup = "{\"groupOp\":\"" + p.groupOp + "\",\"rules\":[",
					gi = 0,
					so;
				val += "";
				if(!$t.p.datatype === 'local') {
			...
			return this.each(function() {
							...
							$.each($t.p.colModel, function() {
								if(this.name === nm && this.editoptions && $.isFunction(this.editoptions.custom_value)) {
									try {
										...
										if(e === "e1") {
											$.jgrid.info_dialog(errors.errcap, "function 'custom_value' " + rp_ge[$(this)[0]].p.msg.novalue, rp_ge[$(this)[0]].p.bClose, {
												styleUI: rp_ge[$(this)[0]].p.styleUI
											});
										} else {
											$.jgrid.info_dialog(errors.errcap, e.message, rp_ge[$(this)[0]].p.bClose, {
												styleUI: rp_ge[$(this)[0]].p.styleUI
											});
										}
									}
							...
							switch(cm[i].edittype) {
								...
								case 'custom':
									try {
										...
										if(e === "e1") {
											$.jgrid.info_dialog(errors.errcap, "function 'custom_value' " + rp_ge[$(this)[0]].p.msg.nodefined, $.rp_ge[$(this)[0]].p.bClose, {
												styleUI: rp_ge[$(this)[0]].p.styleUI
											});
										} else {
											$.jgrid.info_dialog(errors.errcap, e.message, $.rp_ge[$(this)[0]].p.bClose, {
				...
				$("#nData", frmtb2).click(function() {
					...
					if(npos[0] !== -1 && npos[1][npos[0] + 1]) {
						...
						if($("#" + $.jgrid.jqID(npos[1][npos[0] + 1])).hasClass(commonstyle.disabled)) {
							return false;
						}
				...
				$("#pData", frmtb2).click(function() {
					...
					if(ppos[0] !== -1 && ppos[1][ppos[0] - 1]) {
						...
						if($("#" + $.jgrid.jqID(ppos[1][ppos[0] - 1])).hasClass(commonstyle.disabled)) {
							return false;
						}
			...
			return this.each(function() {
				...
				function createData(rowid, obj, tb, maxcols) {
					var nm, hc, trdata, cnt = 0,
						...
						ind = false,
						i,
						tdtmpl = "<td class='CaptionTD form-view-label " + commonstyle.content + "' width='" + p.labelswidth + "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset " + commonstyle.content + "'>&#160;</td>",
						tmpl = "",
					...
					$.ajax($.extend({
						...
						error: function(res, stat, err) {
							...
							if($.isFunction(o.errorfunc)) {
								...
								try {
									$.jgrid.info_dialog(errors.errcap, '<div class="' + error + '">' + rT + '</div>', edit.bClose, {
										buttonalign: 'right',
										styleUI: $t.p.styleUI
									});
								} catch(e) {
	...
	$.jgrid.extend({
		...
		columnChooser: function(opts) {
			var self = this,
				...
				fixedCols = [],
				dopts, mopts, $dialogContent, multiselectData, listHeight,
		...
		addSubGrid: function(pos, sind) {
			return this.each(function() {
				...
				var len = ts.rows.length,
					i = 1,
					hsret, ishsg = $.isFunction(ts.p.isHasSubGrid);
	...
	$.jgrid.extend({
		setTreeNode: function(i, len) {
			return this.each(function() {
				...
				var expCol = $t.p.expColInd,
					...
					lft, rgt, curLevel, ident, lftpos, twrap,
					ldat, lf,
					common = $.jgrid.styleUI[($t.p.styleUI || 'jQueryUI')].common,
					index = i;
		...
		setTreeGrid: function() {
			return this.each(function() {
				...
				$t.p.treeIcons = $.extend({
					plus: pico,
					minus: classes.icon_minus,
					leaf: classes.icon_leaf
				}, $t.p.treeIcons || {});
				if($t.p.treeGridModel === 'nested') {
		...
		getNodeChildren: function(rc, currentview) {
			...
			this.each(function() {
				...
				var i, len = currentview ? this.rows.length : this.p.data.length,
					row;
				switch($t.p.treeGridModel) {
					case 'nested':
						var lftc = $t.p.treeReader.left_field,
