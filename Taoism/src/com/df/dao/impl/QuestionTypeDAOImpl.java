...
public class QuestionTypeDAOImpl implements IQuestionTypeDAO {
	...
	public List<QuestionType> findAll() {
		...
		questionTypeList = sessionFactory.getCurrentSession()
				.createQuery("FROM QuestionType").list();
	...
	public QuestionType getById(Integer id) {
		QuestionType questionType = (QuestionType) sessionFactory
				.getCurrentSession().get(QuestionType.class, id);
		return questionType;
	}
	...
	public QueryResult findAll(Integer firstResult, Integer maxResults) {
		...
		count = (Long) sessionFactory.getCurrentSession()
				.createQuery("SELECT COUNT(*) FROM QuestionType")
				.uniqueResult();
		System.out.println(firstResult + "---" + maxResults + "---" + count);
		// 查询一页的数据列表
		questionTypeList = sessionFactory.getCurrentSession()
				.createQuery("FROM QuestionType").setFirstResult(firstResult)
